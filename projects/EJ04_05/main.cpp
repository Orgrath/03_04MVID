...
void render(uint32_t VAO, const Shader& shader, uint32_t tex1, uint32_t tex2, float ctrlMag) {
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        handleInput();
        std::vector<std::pair<int, int>> key = Input::instance()->getKeys();
        ctrlMag = (key);
        //render(VAO, shader, tex1, tex2);
        render(VAO, shader, tex1, tex2, ctrlMag);
        window->frame();
    }
