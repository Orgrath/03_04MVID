...
void render(const Geometry& quad, const Geometry& cube, const Geometry& teapot, const Geometry& visor, const Shader& s_phong, const Shader& s_fbo,
    ...
    s_phong.set("light.diffuse", 0.3f, 0.3f, 0.3f);
    ...
    s_fbo.set("screenTexture", 1);
    s_fbo.set("horizontal", false);
...
int main(int, char* []) {
    ...
    RenderFBO renderfbo1(resolutionWidth, resolutionHeight);
    RenderFBO renderfbo2(resolutionWidth, resolutionHeight);
    ...
    while (window->alive()) {
        ...
        render(quad, cube, teapot, visor, s_phong, s_fbo, t_albedo, t_specular, renderfbo1.getFBO(), renderfbo1.getTextureColor(), renderfbo2.getFBO(), renderfbo2.getTextureColor());

        window->frame();
    }

    renderfbo1.DeleteFBO();
    renderfbo2.DeleteFBO();
