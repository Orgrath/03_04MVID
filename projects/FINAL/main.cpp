...
void handleInput(Player player) {
    ...
    if (input->isKeyPressed(GLFW_KEY_UP)) {
        player.handleKeyboard(Player::Movement::Forward);
    }
    if (input->isKeyPressed(GLFW_KEY_LEFT)) {
        player.handleKeyboard(Player::Movement::Left);
    }
    if (input->isKeyPressed(GLFW_KEY_RIGHT)) {
        player.handleKeyboard(Player::Movement::Right);
    }
    if (input->isKeyPressed(GLFW_KEY_DOWN)) {
        player.handleKeyboard(Player::Movement::Backward);
    }
}
...
void renderScene(const Shader& shader, const Shader& base,
    ...
    const Geometry& quad, Player& player,
...
int main(int, char* []) {
	...
	const Quad fondo(1.0f);
	const Quad hitbox(1.0f);
    Player player(hitbox, 0.0f, 0.0f, 0.0f);
