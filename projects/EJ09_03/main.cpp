...
void render(const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
    ...
    s_phong.set("dirLight.ambient", 0.00001f, 0.00001f, 0.00001f);
    s_phong.set("dirLight.diffuse", 0.0001f, 0.0001f, 0.0001f);
    s_phong.set("dirLight.specular", 0.005f, 0.005f, 0.005f);
    ...
    for (uint32_t i = 0; i < sizeof(pointLightPositions); ++i) {
        ...
        s_phong.set((lightName + "ambient").c_str(), 0.01f, 0.0f, 0.0f);
    ...
    for (uint32_t i = 0; i < sizeof(spotLightPositions); ++i) {
        ...
        s_phong.set((lightName + "direction").c_str(), spotlightDir);
        s_phong.set((lightName + "ambient").c_str(), 0.1f, 0.1f, 0.1f);
        s_phong.set((lightName + "diffuse").c_str(), 0.2f, 0.2f, 0.2f);
        s_phong.set((lightName + "specular").c_str(), 0.5f, 0.5f, 0.5f);
        s_phong.set((lightName + "constant").c_str(), 1.0f);
        s_phong.set((lightName + "linear").c_str(), 0.1f);
        s_phong.set((lightName + "quadratic").c_str(), 0.06f);
        s_phong.set((lightName + "cutOff").c_str(), glm::cos(glm::radians(15.0f)));
